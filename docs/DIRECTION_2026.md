...
- 본 사업은 국산 스택(NCP + 하이퍼클로바X + 업스테이지, self-hosted Supabase, 국산 AI 직접 호출)으로 수행사가 재구축한다.
...
- **이해상충 원칙 유지**: webfortd는 위원장 자문·취미 영역이지 본 사업 개발비 수령 대상이 아니다(직접 수주 안 함). 개발사(알리의 접근성 연구소)·평가사(에스앤씨랩) 분리 구도와 충돌하지 않는다.
...
장교조 `17. 교육부 및 교육청 등 정책연구/2026년 교육부 정책연구/[과제 5[ 정보 지원 웹페이지 개발 및 운영/CLAUDE.md` §`webfortd 재포지셔닝 전략` (2026-06-05 확정). 사업 측면 의사결정 트레일·이해상충 제약은 그쪽이 정본.
