...
- 실측 정정(2026-08-04 운영 셋업): 설계 당시 "보호 규칙 없음" 판정은 admin 권한 없는 토큰의 404 응답을 오독한 것으로, 실제로는 classic 보호(PR 필수 + validate 필수 체크, 2026-05 설정)가 존재해 편집기의 master 직행 커밋을 거부했다(실호출 게이트가 검출). 운영 셋업에서 classic 보호를 삭제하고 ruleset `master-protect`(force push·브랜치 삭제 금지)로 대체해 직행 커밋이 성립한다.
