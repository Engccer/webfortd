...
- **시나리오 A 1차 트랙**(2026-05-27 중부대 합의): 본 마일스톤이 완료되면 webfortd가 *시범 모델 + 접근성 게이트 통과 + 검색 가시성*까지 갖춰진 상태로 개발 후보 업체·접근성 평가 기관의 견적 검증 자료가 된다.
- **외부 접근성 평가 사전 준비**: axe-core CI는 *자체 자동 회귀 가드*이지 *정식 평가의 대체*가 아니다. 정식 평가는 외부 접근성 평가 기관이 외부 기준으로 수행. 본 axe-core는 *개발 중 회귀 조기 차단*용.
