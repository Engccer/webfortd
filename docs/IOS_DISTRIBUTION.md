...
- **개인 가입**: 가장 빠르다. 다만 App Store의 판매자명(seller name)에 **가입자의 법적
  ...
  ("장애인교원 교육전념 여건 지원 사업의 시범 자산")과 맞는지 먼저 판단이 필요하다. 결제는 지역에 따라 현지 통화로 결제 가능하다.
...
| 앱 스크린샷(6.9"·6.5" 클래스) | 미제작 | 이 문서 §5 시뮬레이터 스모크 스크린샷(`/tmp/webfortd-m4-*.png`)은 개발 검증용이라 그대로 제출용으로 쓸 수 없다. 실제 기기/시뮬레이터에서 App Store 심사 규격에 맞춰 다시 촬영해야 한다. 6.9인치 제공 시 6.5인치는 자동 스케일다운되므로 6.9인치 1세트면 충분하다. 제출 시점에 App Store Connect의 최신 스크린샷 규격(픽셀 치수)을 다시 확인할 것. Apple이 주기적으로 지원 기기 세대를 갱신한다 |
