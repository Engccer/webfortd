...
- **음성 받아쓰기 gildongmu 이식 (2026-07-18)**: ① 웹 전면 개선 — Web Audio 효과음 3종(상승=시작·하강=정지·단음=취소, 기존 무음 no-op useSound 대체), useVoiceRecorder 견고화(오류 코드 계약·더블탭 잠금·언마운트 가드·fetch 취소), 권한 사전 모달 삭제(getUserMedia 네이티브 단일 경로), 시작/정지 음성 안내 제거(효과음+aria-label 변화가 상태 신호, 120초 마일스톤 안내는 유지), transcribe 인식 실패 400→422. ② iOS 채팅 받아쓰기 신설 — SpeechService(iOS 26 SpeechAnalyzer 온디바이스 ko-KR, 서버 왕복 0) + ChatView 마이크 버튼(전사 append). 리뷰 파인딩 3건(취소 경합 레이스) fix 반영. 정본: spec `docs/superpowers/specs/2026-07-18-voice-dictation-gildongmu-port-design.md`. 잔여: 위원장 실 마이크 스모크(웹 효과음·Esc 취소·재포커스 / iOS 버튼 토글·권한 프롬프트). ※ iOS 레이스 fix(세대 토큰·stopping 상호 배제)는 gildongmu 원본에도 동일 결함 — 백포트 권장(미실행).
