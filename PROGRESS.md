...
- **🔶 iOS WhatsApp식 홀드 받아쓰기 이식 (2026-07-20)**: gildongmu 실기기 합격(2026-07-20) 계약 이식 — `HoldDictationButton` 신설(UIKit 인식기 계층, 경합 가드 3종, interrupting 무음 통지, 정적 라벨), 채팅=홀드+잠금+취소 풀 계약(릴리스=초안 병합 즉시 전송)+"텍스트 지우기" 버튼, 위키 검색=홀드 단일 동작. 채팅 리스트·BlockRenderer LazyVStack→eager VStack(gildongmu 실기기 확정 CPU 무한 루프 선제 제거). 리뷰 P1 fix(홀드 중 탭 이탈 늦은 전사 가드). Kit 49·시뮬레이터 빌드 그린, 실기기 배포. **머지 게이트 잔여: 위원장 VoiceOver 실측**(홀드 시작 시 힌트 낭독 즉시 중단 → 잠금 통지·이어쓰기 → 병합 전송 → 왼쪽 취소 → 짧은 탭 안내). 상세: CHANGELOG 2026-07-20.
