...
- **2층 마크다운 정본 v4 4종 완비(2026-08-28)**: `data/source-md/*_fused_v4_*.md`(2023 최종보고서 HWP 정본 경로 + 인쇄 책자 3종 하이브리드). 3층 485건 재생성(주소 체계 변경·원자화 규칙 개정, 8/27 메모 §4·§6 5단계)은 미착수 — 현재 `content/`는 v3 기반이다.
...
4. **3층 재생성(BACKLOG C5)**: `scripts/decompose-source.ts` 규칙 개정 + 슬러그 체계 변경 + 485건 재생성 + 대응표·회귀 표(8/27 메모 §4·§5, 8/28 원자화 설계 메모). 이어서 콘텐츠(BACKLOG §C): FAQ 9건 검수·publish, 큐레이션, 이미지 검수 큐 79건.
