...
- **✅ 음성 받아쓰기 트랙 완결 (2026-07-18, PR #103·#104)**: ① 웹 전면 gildongmu화 — Web Audio 효과음 3종, useVoiceRecorder 견고화(오류 코드 계약·더블탭 잠금·언마운트 가드·fetch 취소), 권한 사전 모달 삭제, 시작/정지 음성 안내 제거(효과음+aria-label 변화가 상태 신호, 120초 마일스톤 안내 유지), transcribe 422. ② iOS 채팅 받아쓰기 신설 — SpeechService(iOS 26 SpeechAnalyzer 온디바이스 ko-KR, 서버 왕복 0) + ChatView 마이크 버튼(append). ③ iOS 위키 검색 받아쓰기 — 정지=쿼리 입력+즉시 검색(검색 전용 계약) + 탭 가시성 가드. 리뷰 fix 3건(취소 경합 레이스). production READY·실스모크 200, iPhone 13 Pro 배포, **위원장 실 마이크 스모크 통과(채팅·위키 검색)**. 정본: spec `docs/superpowers/specs/2026-07-18-voice-dictation-gildongmu-port-design.md`, CHANGELOG 2026-07-18. 잔여 후속: gildongmu 원본 SpeechService에 레이스 fix 2건(세대 토큰·stopping) 백포트 권장(미실행).
